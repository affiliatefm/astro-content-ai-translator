...
async function loadConfig() {
    ...
    if (!configContent) {
        throw new Error("No astro.config found.\n\n" +
    ...
    if (locales.length === 0) {
        throw new Error("No locales found in astro.config.\n\n" +
 ...
 */
function resolveVariable(content, varName, fileDir) {
    // First, check if defined as array in this file
    const localMatch = content.match(new RegExp(`(?:const|let|var)\\s+${varName}\\s*=\\s*\\[([^\\]]+)\\]`));
    if (localMatch) {
        return localMatch[1]
            .match(/["']([^"']+)["']/g)
            ?.map((s) => s.replace(/["']/g, "")) || [];
    ...
    const reExportMatch = content.match(new RegExp(`(?:export\\s+)?const\\s+${varName}\\s*=\\s*(\\w+)\\s*;`));
 ...
 */
function resolveVariableString(content, varName, fileDir) {
        ...
        if (addIntegration) {
            ...
            if (integrationsMatch) {
                ...
                const newIntegrations = existing
                    ...
                    : "aiTranslator()";
                newContent = newContent.replace(/integrations:\s*\[([^\]]*)\]/, `integrations: [${newIntegrations}]`);
            }
...
`);
    ...
    const proceed = await confirm(`\n  Proceed with translation? (${toTranslate.length} file(s), ~${formatCost(est.estimatedCostUSD)})`, true);
    if (!proceed) {
    ...
    const alternatesUpdatedFiles = new Set();
