 ...
 */
function parseFrontmatterStructure(raw) {
    const lines = [];
 ...
 */
function extractRawFrontmatter(raw) {
 ...
 */
function buildTranslatedFrontmatter(sourceRawFm, sourceFm, translatedFields, aiMetadata) {
 ...
 */
function updateFrontmatterField(raw, fieldName, newValue) {
 ...
 */
export async function scanContent(config) {
...
function getLocaleFromPath(relativePath, config) {
...
function parseTranslateTo(value, sourceLocale, config) {
...
function getBasePath(relativePath, locale, config) {
    if (locale === config.defaultLocale)
        return relativePath;
    return relativePath.replace(new RegExp(`^${locale}/`), "");
}
function getTargetPath(source, targetLocale, config) {
...
// TRANSLATOR
// =============================================================================
async function translateContent(source, targetLocale, config, openai) {
...
- Only output the translation, no explanations`;
    ...
    for (const field of translatableFields) {
        if (typeof source.frontmatter[field] === "string") {
            fieldsToTranslate[field] = source.frontmatter[field];
        }
    }
    let userPrompt = "";
...
function parseTranslationResponse(response, originalFields) {
    const frontmatter = {};
 ...
 */
export async function estimate(config, options = {}) {
    ...
    if (options.file) {
        filesToProcess = filesToProcess.filter((s) => s.relativePath.includes(options.file) || basename(s.path).includes(options.file));
    }
    const files = [];
    let totalInputTokens = 0;
    let totalOutputTokens = 0;
    for (const source of filesToProcess) {
        if (!source.translateTo || source.translateTo.length === 0)
            continue;
        for (const targetLocale of source.translateTo) {
            ...
            // Check by permalink
            const alternates = source.frontmatter.alternates;
            if (alternates?.[targetLocale] && !alternates[targetLocale].startsWith("http")) {
                const existing = await findFileByPermalink(join(config.root, config.contentDir), targetLocale, alternates[targetLocale], config);
 ...
 */
export async function translate(config, options = {}) {
    const results = [];
    // Check API key
    ...
    for (const { source, targetLocale, targetRelPath, targetPath } of translations) {
        ...
        try {
            ...
            // Build translated frontmatter preserving source structure
            const translatedFm = buildTranslatedFrontmatter(source.rawFrontmatter, source.frontmatter, {
                ...translated.frontmatter,
                // Fallback to source title if not translated
                title: translated.frontmatter.title || source.frontmatter.title || "",
            }, aiMetadata);
            const output = `---\n${translatedFm}\n---\n\n${translated.content}`;
...
export async function status(config) {
    ...
    const filesToTranslate = sources.filter((s) => s.translateTo !== false && Array.isArray(s.translateTo) && s.translateTo.length > 0);
    ...
    for (const source of filesToTranslate) {
        ...
        for (const locale of targets) {
            ...
            if (existsSync(fullPath)) {
                ...
                const existing = await findFileByPermalink(join(config.root, config.contentDir), locale, alternates[locale], config);
                if (existing) {
                    found = true;
 ...
 */
function updateFileAlternates(filePath, locale, permalink) {
    ...
    const alternates = frontmatter.alternates;
 ...
 */
async function syncAlternatesAcrossLocales(config, results) {
    const updatedFiles = [];
    const contentDir = join(config.root, config.contentDir);
    ...
    for (const [sourcePath, newLocales] of sourceGroups) {
        ...
        if (!sourceFm.alternates)
            continue;
        const sourceAlternates = sourceFm.alternates;
        // Collect all known alternates (from source)
        const allAlternates = { ...sourceAlternates };
        ...
        for (const locale of config.locales) {
            ...
            if (!fm.alternates)
                continue;
            const currentAlts = fm.alternates;
...
async function findFileByPermalink(contentDir, locale, permalink, config) {
